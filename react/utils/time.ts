...
export const parseTimeRemaining = (totalSeconds: number): TimeSplit => {
  const days = Math.floor(totalSeconds / SECONDS_IN_DAY) // Calculate days
  const hours = Math.floor((totalSeconds % SECONDS_IN_DAY) / SECONDS_IN_HOUR) // Calculate hours within 0-23
  ...
  return {
    days: fillWithZero(2, days), // Assuming fillWithZero can handle days
